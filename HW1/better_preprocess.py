    ...
    with open(data_name, 'r') as f:
        for i, line in enumerate(f):
            ...
            for word in words_set:
                tf = float(words.count(word))/(len(words)+1)
...
def main(arguments):
    ...
    valid_output = valid_output.reshape(valid_output.shape[0], 1)

    clf = LogisticAT()
    clf.fit(X_train, Y_train)
    print 1 + clf.score(X_test)
    """
    ...
    """
    p = clf.predict(test_input)
    with open("test.csv", "w") as f:
        f.write("ID,Category\n")
        for i in range(0, len(p+1)):
            f.write(str(i+1) + "," + str(p[i]) + "\n")
    
    with open("test2.csv", "w") as f:
        f.write("ID,Category\n")
        for i in range(0, len(p+1)):
            f.write(str(i+1) + "," + str(p2[i]) + "\n")
    """
